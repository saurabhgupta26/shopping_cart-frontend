...
export default function Main() {
  const persistDataInLocalStorage = JSON.parse(localStorage.getItem("shoppingCart"));
  ...
  useEffect(() => {
    setDiscount(subTotal > Data.discount.minTotal ? (subTotal * Data.discount.discountPercentage) / 100 : 0)
  }, [subTotal])
  ...
  const persistState = () => {
    localStorage.setItem("shoppingCart", JSON.stringify({products,isValid, subTotal, discount, pinData}))
  }

  const qtyIncHandler = (id) => {
    setProducts({...products, [id]:{ ...products[id],qty: products[id].qty+ 1} })
              ...
              return (
                ...
                <div className="d-flex justify-content-between my-4">
                  <div className="product_data t_head d-flex align-items-center">
                    ...
                    <p>{item.desc}</p>
                    {item.planLink ? (
                      <a href={item.planLink} className="d-block"> View Plans </a>
                    ) : (
                      <> </>
                    )}
                    </div>
          ...
          <div className="d-flex justify-content-between px-5">
            ...
            <div className="d-inline-block w-40">
              <div className="delivery_check">
                ...
                <div className="delivery_option d-flex justify-content-between my-4">
                  {isValid ? <>  <div className="d-flex justify-content-between"> 
                  ...
                  <p className="h7 w-100">Estimated delivery time is {pinData.estimatedDays.min} - {pinData.estimatedDays.max} days</p>
                  </div></> : null}
            ...
            <div className="main_total d-inline-block w-40">
              ...
              <div className="d-flex justify-content-between my-4">
                ...
                <p> {pinData && pinData.deliveryPrice !== 0 ? pinData.deliveryPrice + " $": "Free" } </p>
                </div>
                <div className="d-flex justify-content-between my-5">
                <p>Order Total</p>
                <p className="bolder text-dark h4"> {subTotal - discount + (pinData && pinData.deliveryPrice ? pinData.deliveryPrice: 0)} $</p>
                </div>
                <div className="d-flex justify-content-between align-items-center">
                 

                <a href="##" className="text-primary bold">CONTINUE SHOPPING</a>
                <button className="btn btn-primary px-4 py-3 bold rounded-pill" disabled={isValid && subTotal === 0} onClick={(e) => {
                  e.preventDefault();
                  alert("Your total is: " + (subTotal - discount + (pinData && pinData.deliveryPrice ? pinData.deliveryPrice: 0) + "$"))
                }}>CHECKOUT</button>
